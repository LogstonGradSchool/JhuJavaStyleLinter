...
public abstract class AbstractLanguageServer {
   ...
   private final class JhuServerRunnable implements Runnable {
      @Override
      public void run() {
         LOGGER.info("Starting Jhu Language Server...");
         ...
         if (!Thread.currentThread().isInterrupted()) {
            LOGGER.info("Jhu Language Server - Client vanished...");
         }
      }
   }
   ...
   private JhuTextDocumentService textDocumentService;
   ...
   public int startServer() {
      LOGGER.warn("STARTED YO");
      runner =
         new Thread(new JhuServerRunnable(), "Jhu Language Client Watcher");
      runner.start();
      return 0;
   }

   /**
    * Checks whether the parent process is still running. If not, then we assume
    * it has crashed, and we have to terminate the Jhu Language Server.
    *
   ...
   protected boolean parentProcessStillRunning() {
      ...
      if (OS.indexOf("win") != -1) { // && "x86".equals(ARCH)
         command = "cmd /c \"tasklist /FI \"PID eq " + parentProcessId
            + "\" | findstr " + parentProcessId + "\"";
      } else {
   ...
   public void stopServer() {
      ...
      if (runner != null) {
         ...
         LOGGER.info(
            "Request to stop the server has been received but it wasn't started.");
      }
   }
    ...
    */
   public JhuTextDocumentService getTextDocumentService() {
    ...
    */
   protected void setTextDocumentService(
      JhuTextDocumentService textDocumentService) {
